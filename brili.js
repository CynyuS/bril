...
var Heap = /** @class */ (function () {
    ...
    Heap.prototype.freeKey = function (_key) {
        return;
    };
 ...
 */
function evalInstr(instr, state, isMain) {
    if (isMain) {
    ...
    switch (instr.op) {
        ...
        case "set": {
            var shadowVal = getArgument(instr, state.env, 1);
            var shadowDest = instr.args[0];
        ...
        case "get": {
            ...
            var val = state.ssaEnv.get(instr.dest);
            if (val === undefined) {
                throw error("get without corresponding set for ".concat(instr.dest));
            }
            else {
                state.env.set(instr.dest, val);
            }
...
function evalFunc(func, state) {
    var isMain = func.name == "main";
    var visitedLabels = new Set();
    var isTracing = isMain;
    for (var i = 0; i < func.instrs.length; ++i) {
        ...
        if ("op" in line) {
            ...
            // Run an instruction.
            var action = evalInstr(line, state, isMain);
